...
/**
 * The compiler that compiles a given physical plan into a DAG of MapReduce
 * operators which can then be converted into the JobControl structure.
 * 
 * Is implemented as a visitor of the PhysicalPlan it is compiling.
 * 
 * Currently supports all operators except the MR Sort operator
 * 
 * Uses a predecessor based depth first traversal. To compile an operator, first
 * compiles the predecessors into MapReduce Operators and tries to merge the
 * current operator into one of them. The goal being to keep the number of
 * MROpers to a minimum.
 * 
 * It also merges multiple Map jobs, created by compiling the inputs
 * individually, into a single job. Here a new map job is created and then the
 * contents of the previous map plans are added. However, any other state that
 * was in the previous map plans, should be manually moved over. So, if you are
 * adding something new take care about this. Ex of this is in
 * requestedParallelism
 * 
 * Only in case of blocking operators and splits, a new MapReduce operator is
 * started using a store-load combination to connect the two operators. Whenever
 * this happens care is taken to add the MROper into the MRPlan and connect it
 * appropriately.
 * 
 * 
 */
public class MRCompiler extends PhyPlanVisitor {
    ...
    private static final String ROLLUP_PARTITIONER = RollupH2IRGPartitioner.class
            .getName();
    private static final String ROLLUP_AUTO_PIVOT_PARTITIONER = RollupH2IRGAutoPivotPartitioner.class.getName();
    //private static final String HASH_PARTITIONER = HashPartitioner<K2, V2>.class.getName();
    ...
    public MRCompiler(PhysicalPlan plan, PigContext pigContext)
        ...
        fileConcatenationThreshold = Integer.parseInt(pigContext
                .getProperties().getProperty(FILE_CONCATENATION_THRESHOLD,
                        "100"));
    ...
    public void aggregateScalarsFiles() throws PlanException, IOException {
        ...
        Configuration conf = ConfigurationUtil.toConfiguration(pigContext
                .getProperties());
        ...
        for (MapReduceOper mrOp : mrOpList) {
            for (PhysicalOperator scalar : mrOp.scalars) {
                ...
                if (scalar instanceof POStore) {
                    ...
                    if (!hasSeen
                            && combinable
                            && (mro.reducePlan.isEmpty() ? hasTooManyInputFiles(
                                    mro, conf)
                                    : (mro.requestedParallelism >= fileConcatenationThreshold))) {
                        PhysicalPlan pl = mro.reducePlan.isEmpty() ? mro.mapPlan
                                : mro.reducePlan;
                        ...
                        MapReduceOper catMROp = getConcatenateJob(newSpec, mro,
                                newSto);
    ...
    public MROperPlan compile() throws IOException, PlanException,
        ...
        // get all stores and nativeMR operators, sort them in order(operator
        // id)
        // and compile their plans
        List<POStore> stores = PlanHelper.getPhysicalOperators(plan,
                POStore.class);
        List<PONative> nativeMRs = PlanHelper.getPhysicalOperators(plan,
                PONative.class);
        ...
        //Check if this plan is an optimized plan for rollup
        //by checking if there is PORollupH2IRGForEach in reduce
        //plan or not
        for (MapReduceOper check : MRPlan)
            if (PlanHelper.getPhysicalOperators(check.reducePlan,
                    PORollupH2IRGForEach.class).size() != 0)
                try {
                    //User did not specify the pivot position, so, we
                    //perform an autorollup job
                    if(PlanHelper.getPhysicalOperators(check.reducePlan,
                            PORollupH2IRGForEach.class).get(0).getPivot() < 0)
                        doAutoRollupJob(check);
                    else {
                        doAutoRollupJob2(check);
                    }
                    /*else {
                        POForEach pfe = PlanHelper.getPhysicalOperators(check.mapPlan, POForEach.class).get(0);
                        POLocalRearrange plr = PlanHelper.getPhysicalOperators(check.mapPlan, POLocalRearrange.class).get(0);

                        PORollupSampling pfe2 = null;
                        try {
                            pfe2 = _createForEachWithGrpProj(pfe, plr.getKeyType());
                        } catch (CloneNotSupportedException e1) {
                            // TODO Auto-generated catch block
                            e1.printStackTrace();
                        }
                        check.mapPlan.replace(pfe, pfe2);
                        POUserFuncRollupSample puf = (POUserFuncRollupSample)pfe2.getInputPlans().get(0).getLeaves().get(0);
                        List<PhysicalOperator> pufIP = puf.getInputs();
                        for (int i = 0; i < pufIP.size(); i++) {
                            POProject po = (POProject)pufIP.get(i).getInputs().get(0);
                            po.setColumn(i);
                        }
                    }*/
    ...
    /**
     * Transform the original rollup job to two job: sampling job, and
     * the rollup job with the pivot provided from the sampling job
     * @param check
     * @throws IOException
     * @throws PlanException
     * @throws VisitorException
     ...
     */
    private void doAutoRollupJob(MapReduceOper check) throws IOException,
            PlanException, VisitorException, CloneNotSupportedException {

        //Get the POForEach operator in Map plan of the original job
        List<POForEach> lfe = PlanHelper.getPhysicalOperators(check.mapPlan,
                POForEach.class);

        MapReduceOper newplan = getMROp();

        //Create temp file for storing intermediate data
        ...
        List<PhysicalOperator> lpre = null;

        for (POForEach tmp : lfe) {
            if (tmp.getAlias().equals("cube")) {
                //Get the predecessor operator of the POForEach
                //that invoveled in the rollup
                lpre = check.mapPlan.getPredecessors(tmp);
                break;
            }
        }

        //If the predecessor operator is a Load, we use this Load operator
        //for both sampling job and rollup job. After compiling, the original
        //job will become two jobs.
        if (lpre.get(0) instanceof POLoad) {
            POLoad pre = (POLoad) lpre.get(0);
            
            //Mark the job as the rollup sample job
            check.markRollup1();
            //Add the rollup job below this (sampling) job.
            doAddIRG1(pre, fSpec, check);
            //Change the sample job to the count job
            changeSamplingJobToCount(check);
            
            //Mark the PORollupH2IRGForEach operator of sampling job be sample
            List<PORollupH2IRGForEach> lporfe = PlanHelper.getPhysicalOperators(
                    check.reducePlan, PORollupH2IRGForEach.class);
            lporfe.get(0).markSampler();
        }
        //If the predecessor operator is not a Load, we store the above result to
        //a temporary file. This temporary file will be used as input for the sampling
        //job and rollup job. After compiling, the original job will become three jobs.
        else 
        {
            //Store the result of the jobs that were above the rollup job 
            newplan.mapPlan = check.mapPlan.clone();

            check.mapPlan.trimBelow(lpre.get(0));

            check.mapPlan.addAsLeaf(str);
            check.setMapDone(true);

            //Create sampling job
            MapReduceOper old = check;

            doAddSamplingJob(fSpec, old, newplan);
            //Create rollup job.
            doAddIRG1(null, fSpec, newplan);
            //Change the sampling job to the count job
            changeSamplingJobToCount(newplan);
        }
    }
     ...
     * @throws CloneNotSupportedException
     */
    private void doAutoRollupJob2(MapReduceOper check) throws IOException,
            PlanException, VisitorException, CloneNotSupportedException {
    ...
    /**
     * Create sampling job for auto-rollup
     * @param fSpec
     * @param prevJob
     * @param samp
     * @throws IOException
     ...
     * @throws CloneNotSupportedException
     */
    private void doAddSamplingJob(FileSpec fSpec, MapReduceOper prevJob,
            MapReduceOper samp) throws IOException, PlanException,
            VisitorException, CloneNotSupportedException {
        //Get the load and store operator
        POLoad pl = getLoad();
        pl.setLFile(fSpec);
        POStore str = getStore();
        str.setSFile(fSpec);

        //set the parallelism level as the original one.
        samp.requestedParallelism = prevJob.requestedParallelism;
        samp.runtimeParallelism = prevJob.runtimeParallelism;
        
        samp.mapPlan.add(pl);
        MRPlan.add(samp);
        MRPlan.connect(prevJob, samp);

        //set the custom partitioner for this sampling job
        //this job only uses one reducer but we set for later checking
        //in JobControlCompiler
        samp.customPartitioner = ROLLUP_PARTITIONER;

        //Create sampling job. This samplign job is also a rollup-job
        //which is used for estimate the "best" pivot position. So we make
        //it similarly as the rollup job of the original one.
        List<POForEach> nlfe = PlanHelper.getPhysicalOperators(samp.mapPlan,
                POForEach.class);

        for (POForEach tmp : nlfe) {
            if (tmp.getAlias().equals("cube")) {
                samp.mapPlan.trimAbove(tmp);
                break;
            }
        }
        samp.setMapDone(true);
        PhysicalOperator tmp = prevJob.reducePlan.getRoots().get(0);
        PhysicalOperator tmp2 = tmp.clone();
        PhysicalOperator tmp3 = null;
        samp.reducePlan.add(tmp2);
        List<PhysicalOperator> succ = prevJob.reducePlan.getSuccessors(tmp);
        while (succ != null) {
            tmp = succ.get(0);
            tmp3 = tmp.clone();
            if (tmp3 instanceof PORollupH2IRGForEach)
                ((PORollupH2IRGForEach) tmp3).markSampler();
            samp.reducePlan.add(tmp3);
            samp.reducePlan.connect(tmp2, tmp3);
            tmp2 = tmp3;
            succ = prevJob.reducePlan.getSuccessors(tmp);
        }
        samp.setReduceDone(true);
        prevJob.reducePlan = prevJob.combinePlan;

        prevJob.setReduceDone(true);
        prevJob.setMapDone(false);

        for (POForEach tmpfe : nlfe) {
            if (tmpfe.getAlias().equals("cube")) {
                samp.mapPlan.connect(pl, tmpfe);
                break;
            }
        }
        //Mark this job as Rollup Sampler job
        samp.setMapDone(true);
        samp.markRollupSampler();
    }
     ...
     * @throws CloneNotSupportedException
     */
    private void doAddIRG1(POLoad pre, FileSpec fSpec, MapReduceOper prevJob)
            throws IOException, PlanException, VisitorException,
            CloneNotSupportedException {
        POLoad pl = getLoad();
        //Set input file for Load operator
        if(pre == null) {
            pl.setLFile(fSpec);
        }
        else {
        ...
        //Set output file for Store operator
        FileSpec store = getTempFileSpec();

        //Create "real" rollup job
        MapReduceOper rollup1 = getMROp();
        rollup1.requestedParallelism = prevJob.requestedParallelism;
        rollup1.runtimeParallelism = prevJob.runtimeParallelism;
        rollup1.mapPlan = prevJob.mapPlan.clone();
        rollup1.mapPlan.add(pl);
        MRPlan.add(rollup1);
        MRPlan.connect(prevJob, rollup1);

        //Set custom partitioner
        rollup1.customPartitioner = ROLLUP_AUTO_PIVOT_PARTITIONER;

        List<POForEach> nlfe = PlanHelper.getPhysicalOperators(rollup1.mapPlan,
                POForEach.class);

        for (POForEach tmpfe : nlfe) {
            if (tmpfe.getAlias().equals("cube")) {
                rollup1.mapPlan.trimAbove(tmpfe);
                break;
            }
        }
        ...
        for (POForEach tmpfe : nlfe) {
            if (tmpfe.getAlias().equals("cube")) {
                rollup1.mapPlan.connect(pl, tmpfe);
                break;
            }
        }
        ...
        PhysicalOperator tmp2 = tmp.clone();
        PhysicalOperator tmp3 = null;
        rollup1.reducePlan.add(tmp2);
        List<PhysicalOperator> succ = prevJob.reducePlan.getSuccessors(tmp);
        while (succ != null) {
            tmp = succ.get(0);
            tmp3 = tmp.clone();
            rollup1.reducePlan.add(tmp3);
            rollup1.reducePlan.connect(tmp2, tmp3);
            tmp2 = tmp3;
            succ = prevJob.reducePlan.getSuccessors(tmp);
        }

        List<POStore> sampLd = PlanHelper.getPhysicalOperators(
                prevJob.reducePlan, POStore.class);
//        prevJob.setReduceDone(false);
        FileSpec forRealJob = sampLd.get(0).getSFile();
        String outputFile = forRealJob.getFileName();
        String newFilename = outputFile.concat("-sampling");
//        FileSpec newStore = new FileSpec("/tmp/sampling", null);
        store.changeFileName(newFilename);
        store.changeFuncSpec(forRealJob.getFuncSpec());
        sampLd.get(0).setSFile(store);
        prevJob.setReduceDone(true);
        rollup1.markGroupBy();
        rollup1.setReduceDone(true);
        //mark this job as a rollup job
        rollup1.markRollup1();
        
        POForEach pfe = PlanHelper.getPhysicalOperators(rollup1.mapPlan, POForEach.class).get(0);
        POLocalRearrange plr = PlanHelper.getPhysicalOperators(rollup1.mapPlan, POLocalRearrange.class).get(0);
        ...
        rollup1.mapPlan.replace(pfe, pfe2);
        POUserFuncRollupSample puf = (POUserFuncRollupSample)pfe2.getInputPlans().get(0).getLeaves().get(0);
        List<PhysicalOperator> pufIP = puf.getInputs();
        for (int i = 0; i < pufIP.size(); i++) {
            POProject po = (POProject)pufIP.get(i).getInputs().get(0);
     ...
     * @throws CloneNotSupportedException
     */
    private void doAddIRG2(POLoad pre, FileSpec fSpec, MapReduceOper prevJob)
            throws IOException, PlanException, VisitorException,
            CloneNotSupportedException {
        ...
        String input = outputFile + "-irg1-tmp";
        FileSpec rollup2Load = pl.getLFile();
        FileSpec tmpLoad = getTempFileSpec();
        tmpLoad.changeFuncSpec(rollup2Load.getFuncSpec());
        tmpLoad.changeFileName(input);
        //fSpec.changeFileName(input);
        pl.setLFile(tmpLoad);
        ...
        //Create "real" rollup job
        MapReduceOper rollup2 = getMROp();
        rollup2.requestedParallelism = 1;
        rollup2.runtimeParallelism = 1;
        rollup2.mapPlan = prevJob.mapPlan.clone();
        rollup2.mapPlan.add(pl);
        MRPlan.add(rollup2);
        MRPlan.connect(prevJob, rollup2);

        //Set custom partitioner
        rollup2.customPartitioner = ROLLUP_AUTO_PIVOT_PARTITIONER;

        List<POForEach> nlfe = PlanHelper.getPhysicalOperators(rollup2.mapPlan,
                POForEach.class);

        for (POForEach tmpfe : nlfe) {
            if (tmpfe.getAlias().equals("PORollupSampling")) {
                rollup2.mapPlan.trimAbove(tmpfe);
                break;
            }
        }
        ...
        for (POForEach tmpfe : nlfe) {
            if (tmpfe.getAlias().equals("PORollupSampling")) {
                rollup2.mapPlan.connect(pl, tmpfe);
                break;
            }
        }

/*        List<POLoad> pol = PlanHelper.getPhysicalOperators(rollup2.mapPlan, POLoad.class);
        FileSpec rollup2Load = pol.get(0).getLFile();
        rollup2Load.changeFileName(outputFile + "-irg1-tmp");*/
        
        PhysicalOperator tmp = prevJob.reducePlan.getRoots().get(0);
        PhysicalOperator tmp2 = tmp.clone();
        PhysicalOperator tmp3 = null;
        rollup2.reducePlan.add(tmp2);
        List<PhysicalOperator> succ = prevJob.reducePlan.getSuccessors(tmp);
        while (succ != null) {
            tmp = succ.get(0);
            tmp3 = tmp.clone();
            rollup2.reducePlan.add(tmp3);
            rollup2.reducePlan.connect(tmp2, tmp3);
            tmp2 = tmp3;
            succ = prevJob.reducePlan.getSuccessors(tmp);
        }

        String newFilename = outputFile.concat("-irg1");
//        FileSpec newStore = new FileSpec("/tmp/sampling", null);
        store.changeFileName(newFilename);
        store.changeFuncSpec(forRealJob.getFuncSpec());
        sampLd.get(0).setSFile(store);
        prevJob.setReduceDone(true);
        rollup2.markGroupBy();
        rollup2.setReduceDone(true);
        //mark this job as a rollup job
        rollup2.markRollup2();
    }
    
    /**
     * Before calling this function, the sampling job is the same
     * as the original rollup job. We must make sure this sampling job
     * is only used for counting and estimating the pivot position
     * @param samp
     * @throws IOException
     * @throws PlanException
     * @throws VisitorException
     */
    private void changeSamplingJobToCount(MapReduceOper samp) throws IOException, PlanException,
    VisitorException{
        PORollupH2IRGForEach lfe = PlanHelper
                .getPhysicalOperators(samp.reducePlan,
                        PORollupH2IRGForEach.class).get(0);
        String LONG_SUM = "org.apache.pig.builtin.LongSum";
        //if the rollup job has multiple user-func functions followed,
        //we keep only one function.
        int index = lfe.inputPlans.size() - 1;
        while(lfe.inputPlans.get(index-1).getLeaves().get(0) instanceof POUserFunc) {
            lfe.inputPlans.remove(index);
            index--;
        }
        
        //Update the PORollupH2IRGForEach operator after removing 
        //other user-func functions and keeping only one.
        lfe.getLeaves();
        
        POUserFunc ufClone = null;
        POProject poClone = null;
        
        POProject tmpp = (POProject) lfe.inputPlans.get(0).getRoots().get(0);
        
        try {
            ufClone = ((POUserFunc) lfe.inputPlans.get(1).getLeaves().get(0)).clone();
            POProject originalPo = ((POProject) lfe.inputPlans.get(1).getRoots().get(0));
            poClone = originalPo.clone();
//            if(lfe.inputPlans.get(1).getSuccessors(originalPo).size()!=0) {
//                if (lfe.inputPlans.get(1).getSuccessors(originalPo).get(0) instanceof POProject) {
//                    PhysicalOperator innerPo = (POProject) lfe.inputPlans.get(1).getSuccessors(originalPo).get(0).clone();
//                    List<PhysicalOperator> innerlist = Arrays.asList(innerPo);
//                    poClone.setInputs(innerlist);
//                }
//            }
        ...
        //Our samplign job will do a counting after rollup, so we change the 
        //remained user-func function to a LongSum function.
        if(!ufClone.getFuncSpec().getClassName().equals(LONG_SUM)) {
                ufClone.changeToSumHIISample();
                lfe.inputPlans.remove(1);
                PhysicalPlan uf = new PhysicalPlan();
                uf.add(ufClone);
                uf.add(poClone);
                uf.connect(poClone, ufClone);
                lfe.inputPlans.add(1, uf);
                lfe.getLeaves();
        }

        POForEach pfe = PlanHelper.getPhysicalOperators(samp.mapPlan, POForEach.class).get(0);
        POLocalRearrange plr = PlanHelper.getPhysicalOperators(samp.mapPlan, POLocalRearrange.class).get(0);
    ...
    private POForEach removePOUserFunc(POForEach pfe) throws IOException, PlanException,
    VisitorException, CloneNotSupportedException{
        POForEach pfe3 = new POForEach(createOperatorKey(pfe.getOperatorKey().scope), new ArrayList<PhysicalPlan>());
        pfe3.addOriginalLocation("Load-cast", pfe.getOriginalLocations());
        pfe3.setResultType(DataType.BAG);
        List<PhysicalOperator> lpfe = pfe.getInputPlans().get(0).getRoots();
        int indexProj = 0;
        while(pfe3.getInputPlans().size() < lpfe.size())
            
            for(PhysicalOperator i : lpfe) {
                try {
                    if(((POProject)i).getColumn() == indexProj) {
                        POCast pc = (POCast) pfe.getInputPlans().get(0).getSuccessors(i).get(0).clone();
                        POProject po = (POProject) i.clone();
                        PhysicalPlan tp = new PhysicalPlan();
                        tp.add(pc);
                        tp.add(po);
                        tp.connect(po, pc);
                        pfe3.addInputPlan(tp, false);
                        indexProj++;
                        break;
                    }
                } catch (CloneNotSupportedException e) {
        ...
        PhysicalPlan tp = new PhysicalPlan();
        POProject po = (POProject) pfe.getInputPlans().get(1).getRoots().get(0);
        POCast pc = (POCast) pfe.getInputPlans().get(1).getSuccessors(pfe.getInputPlans().get(1).getRoots().get(0)).get(0).clone();
        tp.add(pc);
        tp.add(po);
        tp.connect(po, pc);
        pfe3.addInputPlan(tp, false);
    ...
    //with POUserFunc inside
    private PORollupSampling _createForEachWithGrpProj(POForEach foreach, byte keyType) throws IOException, PlanException,
    VisitorException, CloneNotSupportedException {
        String scope = foreach.getOperatorKey().scope;
        PORollupSampling newFE = new PORollupSampling(createOperatorKey(scope), new ArrayList<PhysicalPlan>());
        newFE.addOriginalLocation("PORollupSampling", foreach.getOriginalLocations());
        newFE.setResultType(DataType.BAG);
        
        List<PhysicalOperator> lpfe = foreach.getInputPlans().get(0).getRoots();
        POUserFunc puf = (POUserFunc)foreach.getInputPlans().get(0).getLeaves().get(0).clone();
        //POUserFuncRollupSample pufrs = new POUserFuncRollupSample(createOperatorKey(puf.getOperatorKey().scope), -1, 
                //new ArrayList<PhysicalOperator>(), puf.getFuncSpec());

        Object f = PigContext.instantiateFuncFromSpec(puf.getFuncSpec());
        POUserFuncRollupSample pufrs = null;
        if (f instanceof EvalFunc) {
            pufrs = new POUserFuncRollupSample (createOperatorKey(puf.getOperatorKey().scope),-1,
                    null, puf.getFuncSpec(), (EvalFunc) f);
        }
        
        pufrs.setInputs(foreach.getInputPlans().get(0).getLeaves().get(0).getInputs());
        pufrs.addOriginalLocation("POUserFuncRollupSampling", puf.getOriginalLocations());
        pufrs.setResultType(puf.getResultType());
        pufrs.setPivot(puf.getPivot());
        pufrs.setDimensionSize(puf.getDimensionSize());
        pufrs.setRollupFieldIndex(puf.getRollupFieldIndex());
        pufrs.setRollupOldFieldIndex(puf.getRollupOldFieldIndex());
        pufrs.setRollupH2IRGOptimizable(puf.getRollupH2IRGOptimizable());
        pufrs.setChunkSize(puf.getChunkSize());
        pufrs.setRate(puf.getRate());
        
        //pufrs.setFuncSpec(puf.getFuncSpec());
        //pufrs.setOrigFSpec(puf.getOrigFSpec());
        pufrs.setEvalFunc(puf.getFunc());
        
        int indexProj = 0;
        PhysicalPlan grpProjPlan = new PhysicalPlan();
        //while(indexProj < lpfe.size()) {
            
            for(PhysicalOperator i : lpfe) {
                try {
                    //if(((POProject)i).getColumn() == indexProj) {
                    {   POCast pc = (POCast) foreach.getInputPlans().get(0).getSuccessors(i).get(0).clone();
                        POProject po = (POProject) i.clone();
                        po.setColumn(indexProj);
                        grpProjPlan.add(pc);
                        grpProjPlan.add(po);
                        grpProjPlan.connect(po, pc);
                        //newFE.addInputPlan(tp, false);
                        indexProj++;
                        //break;
                    }
                } catch (CloneNotSupportedException e) {
...
//        for (int i = 0 ; i < foreach.getInputPlans().get(0).getRoots().size(); i++) {
//            POProject proj = new POProject(createOperatorKey(scope), 1, i);
...
//        }
        ...
        try {
            ...
            grpProjPlan.addAsLeaf(pufrs);
            newFE.addInputPlan(grpProjPlan, true);
            
            PhysicalPlan grpProjPlan2 = new PhysicalPlan();
            
            if(foreach.getInputPlans().size() > 1) {
                List<PhysicalOperator> lpfe2 = foreach.getInputPlans().get(1).getRoots();
                
                POProject proj = (POProject)lpfe2.get(0).clone();
                
                POCast pc = (POCast) foreach.getInputPlans().get(1).getSuccessors((lpfe2.get(0))).get(0).clone();
                
                grpProjPlan2.add(proj);
                grpProjPlan2.add(pc);
                //proj.setResultType(DataType.INTEGER);
                
                grpProjPlan2.connect(proj, pc);
                
                newFE.addInputPlan(grpProjPlan2, false);
            }
        } catch (Exception e) {
            // TODO Auto-generated catch block
            e.printStackTrace();
        ...
        newFE.setInputs(foreach.getInputs());
        
        return newFE;
    }
    
    private PORollupSampling createForEachWithGrpProj(POForEach foreach, byte keyType) {
        String scope = foreach.getOperatorKey().scope;
        PORollupSampling newFE = new PORollupSampling(createOperatorKey(scope), new ArrayList<PhysicalPlan>());
        newFE.addOriginalLocation("PORollupSampling", foreach.getOriginalLocations());
        newFE.setResultType(DataType.BAG);

        for (int i = 0 ; i < foreach.getInputPlans().get(0).getRoots().size()+1; i++) {
            PhysicalPlan grpProjPlan = new PhysicalPlan();
            POProject proj = new POProject(createOperatorKey(scope), 1, i);
            proj.setResultType(DataType.INTEGER);
            grpProjPlan.add(proj);
        ...
        for (PhysicalOperator op : plan) {
            if (plan.getSoftLinkPredecessors(op) != null) {
                for (PhysicalOperator pred : plan.getSoftLinkPredecessors(op)) {
                    ...
                    if (from == to)
                        continue;
                    if (MRPlan.getPredecessors(to) == null
                            || !MRPlan.getPredecessors(to).contains(from)) {
    ...
    /**
     * Compiles the plan below op into a MapReduce Operator and stores it in
     * curMROp.
     * 
     * @param op
     * @throws IOException
    ...
    private void compile(PhysicalOperator op) throws IOException,
        ...
        if (op instanceof PONative) {
            ...
            if (op instanceof POLoad) {
                ...
                if (p instanceof POStore || p instanceof PONative) {
                    ...
                    int errCode = 2126;
                    String msg = "Predecessor of load should be a store or mapreduce operator. Got "
                            + p.getClass();
                    throw new PlanException(msg, errCode, PigException.BUG);
                }
            ...
            for (PhysicalOperator pred : predecessors) {
                if (pred instanceof POSplit
                    ...
                    compiledInputs[++i] = startNew(((POSplit) pred)
                            .getSplitStore(), splitsSeen.get(pred
                            .getOperatorKey()));
                    continue;
                }
                compile(pred);
            ...
            if (op != null && op instanceof POLoad) {
                if (((POLoad) op).getLFile() != null
                    ...
                    curMROp.UDFs.add(((POLoad) op).getLFile().getFuncSpec()
                            .toString());
            }
            MRPlan.add(curMROp);
            phyToMROpMap.put(op, curMROp);
        ...
        if (op.getRequestedParallelism() > curMROp.requestedParallelism) {
            // we don't want to change prallelism for skewed join due to
            // sampling
    ...
    private MapReduceOper getMROp() {
        return new MapReduceOper(new OperatorKey(scope, nig
                .getNextNodeId(scope)));
    }

    private NativeMapReduceOper getNativeMROp(String mrJar, String[] parameters) {
        return new NativeMapReduceOper(new OperatorKey(scope, nig
                .getNextNodeId(scope)), mrJar, parameters);
    }
    ...
    private POStore getStore() {
        POStore st = new POStore(new OperatorKey(scope, nig
                .getNextNodeId(scope)));
    ...
    /**
     * A map MROper is an MROper whose map plan is still open for taking more
     * non-blocking operators. A reduce MROper is an MROper whose map plan is
     * done but the reduce plan is open for taking more non-blocking opers.
     * 
     ...
     * open. Otherwise, we merge the compiled inputs into a list of MROpers
     * where the first oper is the merged oper consisting of all map MROpers and
     * the rest are reduce MROpers as reduce plans can't be merged. Then we add
     * the input oper op into the merged map MROper's map plan as a leaf and
     * connect the reduce MROpers using store-load combinations to the input
     * operator which is the leaf. Also care is taken to connect the MROpers
     ...
     * creates the map-reduce boundary. So the merged map MROper is closed and
     * its reduce phase is started. Depending on the number of reduce MROpers
     * and the number of pipelines in the map MRoper a Union operator is
     * inserted whenever necessary. This also leads to the possibility of empty
     * map plans. So have to be careful while handling it in the PigMapReduce
     * class. If there are no map plans, then a new one is created as a side
     * effect of the merge process. If there are no reduce MROpers, and only a
     * single pipeline in the map, then no union oper is added. Otherwise a
     * Union oper is added to the merged map MROper to which all the reduce
     * MROpers are connected by store-load combinations. Care is taken to
     * connect the MROpers in the MRPlan.
     * 
     * @param op
     * @throws IOException
     * @throws PlanException
     */
    private void blocking(PhysicalOperator op) throws IOException,
            PlanException {
        if (compiledInputs.length == 1) {
            MapReduceOper mro = compiledInputs[0];
            if (!mro.isMapDone()) {
    ...
    /**
     * Connect the reduce MROpers to the leaf node in the map MROper mro by
     * adding appropriate loads
     * 
     * @param mergedPlans
     *            - The list of reduce MROpers
     * @param mro
     *            - The map MROper
     * @throws PlanException
     * @throws IOException
     */
    private void connRedOper(List<MapReduceOper> mergedPlans, MapReduceOper mro)
     ...
     * 
     * @param fSpec
     *            Temp file to force a store into.
     * @return MR operator that now is finished with a store.
     * @throws PlanException
     */
    private MapReduceOper endSingleInputPlanWithStr(FileSpec fSpec)
            throws PlanException {
        if (compiledInputs.length > 1) {
            int errCode = 2023;
    ...
    /**
     * Starts a new MRoper and connects it to the old one by load-store. The
     * assumption is that the store is already inserted into the old MROper.
     * 
     * @param fSpec
     * @param old
     * @return
     ...
     * @throws PlanException
     */
    private MapReduceOper startNew(FileSpec fSpec, MapReduceOper old)
            throws PlanException {
        POLoad ld = getLoad();
        ld.setLFile(fSpec);
    ...
    private FileSpec getTempFileSpec() throws IOException {
        return new FileSpec(FileLocalizer.getTemporaryPath(pigContext)
                .toString(), new FuncSpec(Utils
                .getTmpFileCompressorName(pigContext)));
    }

    /**
     * Merges the map MROpers in the compiledInputs into a single merged map
     * MRoper and returns a List with the merged map MROper as the first oper
     * and the rest being reduce MROpers.
     * 
     * Care is taken to remove the map MROpers that are merged from the MRPlan
     * and their connections moved over to the merged map MROper.
     * 
     * Merge is implemented as a sequence of binary merges. merge(PhyPlan
     * finPlan, List<PhyPlan> lst) := finPlan,merge(p) foreach p in lst
     * 
    ...
    private List<MapReduceOper> merge(MapReduceOper[] compiledInputs)
        ...
        for (MapReduceOper rmro : remLst) {
            ...
            for (Map.Entry<PhysicalOperator, MapReduceOper> entry : phyToMROpMap
                    .entrySet()) {
    ...
    /**
     * Compiles a split operator. The logic is to close the split job by
     * replacing the split oper by a store and creating a new Map MRoper and
     * return that as the current MROper to which other operators would be
     * compiled into. The new MROper would be connected to the split job by
     * load-store. Also add the split oper to the splitsSeen map.
     * 
     * @param op
     *            - The split operator
     * @throws VisitorException
    ...
    public void visitNative(PONative op) throws VisitorException {
        // We will explode the native operator here to add a new MROper for
        // native Mapreduce job
        try {
            // add a map reduce boundary
            MapReduceOper nativeMROper = getNativeMROp(op.getNativeMRjar(), op
                    .getParams());
    ...
    public void visitLimit(POLimit op) throws VisitorException {
        try {
            ...
            if (!mro.isMapDone()) {
                // if map plan is open, add a limit for optimization, eventually
                // we
                ...
                if (mro.reducePlan.isEmpty()) {
                    ...
                    messageCollector
                            .collect(
                                    "Something in the reduce plan while map plan is not done. Something wrong!",
                                    MessageType.Warning,
                                    PigWarning.REDUCE_PLAN_NOT_EMPTY_WHILE_MAP_PLAN_UNDER_PROCESS);
                }
            } else if (mro.isMapDone() && !mro.isReduceDone()) {
    ...
    public void visitPOForEach(POForEach op) throws VisitorException {
        try {
            ...
            // if (op.getAlias().equals("cube")) {
            // FileSpec fSpec = getTempFileSpec();
            // MapReduceOper mro = endSingleInputPlanWithStr(fSpec);
            // FileSpec quantFile = getTempFileSpec();
            // int rp = op.getRequestedParallelism();
            // Pair<MapReduceOper, Integer> quantJobParallelismPair =
            // getSamplingRollupJob(
            // op, mro, fSpec, quantFile, rp);
            // curMROp = getRollupJob(op, quantJobParallelismPair.first,
            // fSpec, quantFile, quantJobParallelismPair.second);
            // if (plans != null)
    ...
    @Override
    public void visitPORollupH2IRGForEach(PORollupH2IRGForEach op)
            throws VisitorException {
        try {
            nonBlocking(op);
            List<PhysicalPlan> plans = op.getInputPlans();
     ...
     * replicated files that are configured in the POFRJoin operator. It also
     * sets that this is FRJoin job and some parametes associated with it.
     */
    @Override
    public void visitFRJoin(POFRJoin op) throws VisitorException {
        try {
            ...
            for (int i = 0; i < compiledInputs.length; i++) {
                ...
                Configuration conf = ConfigurationUtil
                        .toConfiguration(pigContext.getProperties());
                boolean combinable = !conf.getBoolean("pig.noSplitCombination",
                        false);
                ...
                if (UriUtil.isHDFSFile(fileName)) {
                    ...
                    for (String location : locations) {
                        ...
                        if (fs.exists(path)) {
                            ...
                            List<InputSplit> splits = inf.getSplits(HadoopShims
                                    .cloneJobContext(job));
                            List<List<InputSplit>> results = MapRedUtil
                                    .getCombinePigSplits(splits, HadoopShims
                                            .getDefaultBlockSize(fs, path),
                                            conf);
                            numFiles += results.size();
                        } else {
                            List<MapReduceOper> preds = MRPlan
                                    .getPredecessors(mro);
    ...
    private MapReduceOper getConcatenateJob(FileSpec fSpec, MapReduceOper old,
            POStore str) throws PlanException, ExecException {
    ...
    @Override
    public void visitMergeCoGroup(POMergeCogroup poCoGrp)
            throws VisitorException {
        ...
        List<FuncSpec> funcSpecs = new ArrayList<FuncSpec>(
                compiledInputs.length - 1);
        List<String> fileSpecs = new ArrayList<String>(
                compiledInputs.length - 1);
        List<String> loaderSigns = new ArrayList<String>(
                compiledInputs.length - 1);

        try {
            // Iterate through all the MROpers, disconnect side MROPers from
            // MROPerPlan and collect all the information needed in different
            // lists.
            ...
            for (int i = 0; i < compiledInputs.length; i++) {
                ...
                if (!(rootPOOp instanceof POLoad)) {
                    int errCode = 2172;
                    String errMsg = "Expected physical operator at root to be POLoad. Found : "
                            + rootPOOp.getClass().getCanonicalName();
                    throw new MRCompilerException(errMsg, errCode);
                }
                ...
                if (!(IndexableLoadFunc.class.isAssignableFrom(loadfunc
                        .getClass()))) {
                    int errCode = 2253;
                    throw new MRCompilerException(
            ...
            // operation.
            MapReduceOper baseMROp = phyToMROpMap.get(poCoGrp.getInputs()
                    .get(0));
            ...
            FileSpec idxFileSpec = getIndexingJob(indexerMROp, baseMROp,
            ...
            throw new MRCompilerException(e.getDetailedMessage(), e
                    .getErrorCode(), e.getErrorSource(), e);
    ...
    private FileSpec getIndexingJob(MapReduceOper indexerMROp,
            final MapReduceOper baseMROp,
            final List<PhysicalPlan> mapperLRInnerPlans)
            throws MRCompilerException, PlanException, ExecException,
        ...
        indexerArgs[1] = ObjectSerializer
                .serialize((Serializable) mapperLRInnerPlans);
        ...
        indexerMROp.requestedParallelism = 1; // we need exactly one reducer for
                                              // indexing job.

        // We want to use typed tuple comparator for this job, instead of
        // default
     ...
     * operators, both are used to generate a MR job each. First MR oper is run
     * to generate on-the-fly index on right side. Second is used to actually do
     * the join. First MR oper is identified as rightMROper and second as
     * curMROper.
     * 
     * 1) RightMROper: If it is in map phase. It can be preceded only by POLoad.
     * If there is anything else in physical plan, that is yanked and set as
     * inner plans of joinOp. If it is reduce phase. Close this operator and
     * start new MROper. 2) LeftMROper: If it is in map phase, add the Join
     * operator in it. If it is in reduce phase. Close it and start new MROper.
     */
    ...
    public void visitMergeJoin(POMergeJoin joinOp) throws VisitorException {
        ...
        try {
            if (compiledInputs.length != 2 || joinOp.getInputs().size() != 2) {
                int errCode = 1101;
                throw new MRCompilerException(
                        "Merge Join must have exactly two inputs. Found : "
                                + compiledInputs.length, errCode);
            }
            ...
            if (!rightMROpr.mapDone) {
                ...
                if (!(rightLoader instanceof POLoad)) {
                    int errCode = 2172;
                    String errMsg = "Expected physical operator at root to be POLoad. Found : "
                            + rightLoader.getClass().getCanonicalName();
                    throw new MRCompilerException(errMsg, errCode);
                }
                ...
                if (rightMapPlan.getSuccessors(rightLoader) == null
                    ...
                    rightPipelinePlan.disconnect(root, rightPipelinePlan
                            .getSuccessors(root).get(0));
                ...
                // start new MROper.
                // No need of yanking in this case. Since we are starting brand
                // new MR Operator and it will contain nothing.
            ...
            if (IndexableLoadFunc.class.isAssignableFrom(rightLoadFunc
                    .getClass())) {
                joinOp.setRightLoaderFuncSpec(rightLoader.getLFile()
                        .getFuncSpec());
                joinOp.setRightInputFileName(rightLoader.getLFile()
                        .getFileName());
                udfs.add(rightLoader.getLFile().getFuncSpec().toString());

                // we don't need the right MROper since
                // the right loader is an IndexableLoadFunc which can handle the
                // index
                // itself
                ...
                // join keys
                int numInputs = mPlan.getPredecessors(joinOp).size(); // should
                                                                      // be 2
                ...
                if (!(OrderedLoadFunc.class.isAssignableFrom(loadFunc
                        .getClass()))) {
                    int errCode = 1104;
                ...
                indexerArgs[0] = origRightLoaderFileSpec.getFuncSpec()
                        .toString();
                indexerArgs[1] = ObjectSerializer
                        .serialize((Serializable) rightInpPlans);
                ...
                FileSpec lFile = new FileSpec(rightLoader.getLFile()
                        .getFileName(), new FuncSpec(MergeJoinIndexer.class
                        .getName(), indexerArgs));
                ...
                defaultIndexableLoaderArgs[0] = origRightLoaderFileSpec
                        .getFuncSpec().toString();
                defaultIndexableLoaderArgs[1] = strFile.getFileName();
                defaultIndexableLoaderArgs[2] = strFile.getFuncSpec()
                        .toString();
                defaultIndexableLoaderArgs[3] = joinOp.getOperatorKey().scope;
                defaultIndexableLoaderArgs[4] = origRightLoaderFileSpec
                        .getFileName();
                joinOp.setRightLoaderFuncSpec((new FuncSpec(
                        DefaultIndexableLoader.class.getName(),
                        defaultIndexableLoaderArgs)));
                joinOp.setRightInputFileName(origRightLoaderFileSpec
                        .getFileName());
    ...
    public void visitDistinct(PODistinct op) throws VisitorException {
        try {
            ...
            POProject prjStar = new POProject(new OperatorKey(scope, nig
                    .getNextNodeId(scope)));
            ...
            POPackage pkg = new POPackage(new OperatorKey(scope, nig
                    .getNextNodeId(scope)));
            ...
            POProject prj1 = new POProject(new OperatorKey(scope, nig
                    .getNextNodeId(scope)));
    ...
    public void visitSkewedJoin(POSkewedJoin op) throws VisitorException {
        try {
            if (compiledInputs.length != 2) {
                int errCode = 2255;
                throw new VisitorException("POSkewedJoin operator has "
                        + compiledInputs.length + " inputs. It should have 2.",
                        errCode);
            }
            ...
            Pair<MapReduceOper, Integer> sampleJobPair = getSkewedJoinSampleJob(
                    op, mro, fSpec, partitionFile, rp);
            rp = sampleJobPair.second;

            // set parallelism of SkewedJoin as the value calculated by sampling
            // job
            // if "parallel" is specified in join statement, "rp" is equal to
            // that number
            ...
            MultiMap<PhysicalOperator, PhysicalPlan> joinPlans = op
                    .getJoinPlans();
            List<PhysicalPlan> groups = joinPlans.get(l.get(0));
            // check the type of group keys, if there are more than one field,
            // the key is TUPLE.
            ...
            // if the map for current input is already closed, then start a new
            // job
            if (compiledInputs[0].isMapDone()
                    && !compiledInputs[0].isReduceDone()) {
            ...
            POPartitionRearrange pr = new POPartitionRearrange(new OperatorKey(
                    scope, nig.getNextNodeId(scope)), rp);
            ...
            POGlobalRearrange gr = new POGlobalRearrange(new OperatorKey(scope,
                    nig.getNextNodeId(scope)), rp);
            ...
            for (int i = 0; i < 2; i++) {
                ep = new PhysicalPlan();
                POProject prj = new POProject(new OperatorKey(scope, nig
                        .getNextNodeId(scope)));
                prj.setColumn(i + 1);
            ...
            POForEach fe = new POForEach(new OperatorKey(scope, nig
                    .getNextNodeId(scope)), -1, eps, flat);
    ...
    public void visitSort(POSort op) throws VisitorException {
        try {
            ...
            Pair<MapReduceOper, Integer> quantJobParallelismPair = getQuantileJob(
                    op, mro, fSpec, quantFile, rp);
            curMROp = getSortJob(op, quantJobParallelismPair.first, fSpec,
                    quantFile, quantJobParallelismPair.second, fields);
     ...
     * being a row number, any previous jobs are saved and POCounter is added as
     * a leaf on a map task. If it is not, then POCounter is added as a leaf on
     * a reduce task (last sorting phase).
     **/
    @Override
    ...
    private MapReduceOper getSortJob(POSort sort, MapReduceOper quantJob,
            FileSpec lFile, FileSpec quantFile, int rp,
        ...
        if (fields == null) {
            ...
            /*
             * for (int i : fields) { PhysicalPlan ep = new PhysicalPlan();
             * POProject prj = new POProject(new OperatorKey(scope,
             * nig.getNextNodeId(scope))); prj.setColumn(i);
             * prj.setOverloaded(false); prj.setResultType(DataType.BYTEARRAY);
             * ep.add(prj); eps1.add(ep); }
             */
            ...
            try {
                FindKeyTypeVisitor fktv = new FindKeyTypeVisitor(sort
                        .getSortPlans().get(0));
                fktv.visit();
        ...
        POLocalRearrange lr = new POLocalRearrange(new OperatorKey(scope, nig
                .getNextNodeId(scope)));
        try {
            lr.setIndex(0);
        } catch (ExecException e) {
        ...
        if (limit != -1) {
            POPackageLite pkg_c = new POPackageLite(new OperatorKey(scope, nig
                    .getNextNodeId(scope)));
            ...
            POProject prj_c1 = new POProject(new OperatorKey(scope, nig
                    .getNextNodeId(scope)));
            prj_c1.setColumn(1);
            ...
            eps_c1.add(ep_c1);
            flat_c1.add(true);
            POForEach fe_c1 = new POForEach(new OperatorKey(scope, nig
                    .getNextNodeId(scope)), -1, eps_c1, flat_c1);
            fe_c1.setResultType(DataType.TUPLE);
            mro.combinePlan.addAsLeaf(fe_c1);

            POLimit pLimit = new POLimit(new OperatorKey(scope, nig
                    .getNextNodeId(scope)));
            ...
            POLocalRearrange lr_c2 = new POLocalRearrange(new OperatorKey(
        ...
        POPackageLite pkg = new POPackageLite(new OperatorKey(scope, nig
                .getNextNodeId(scope)));
        ...
        flattened.add(true);
        POForEach nfe1 = new POForEach(new OperatorKey(scope, nig
                .getNextNodeId(scope)), -1, eps2, flattened);
        mro.reducePlan.add(nfe1);
        mro.reducePlan.connect(pkg, nfe1);
        mro.phyToMRMap.put(sort, nfe1);
        if (limit != -1) {
            POLimit pLimit2 = new POLimit(new OperatorKey(scope, nig
                    .getNextNodeId(scope)));
    ...
    private Pair<MapReduceOper, Integer> getQuantileJob(POSort inpSort,
        ...
        sort.addOriginalLocation(inpSort.getAlias(), inpSort
                .getOriginalLocations());

        // Turn the asc/desc array into an array of strings so that we can pass
        // it
        ...
        if (sort.isUDFComparatorUsed) {
            String userComparatorFuncSpec = sort.getMSortFunc().getFuncSpec()
                    .toString();
     ...
     */
    private Pair<MapReduceOper, Integer> getSkewedJoinSampleJob(
            POSkewedJoin op, MapReduceOper prevJob, FileSpec lFile,
            FileSpec sampleFile, int rp) throws PlanException, VisitorException {
        ...
        POSort sort = new POSort(op.getOperatorKey(), op
                .getRequestedParallelism(), null, groups, ascCol, null);
        ...
        PhysicalPlan ep = new PhysicalPlan();
        POUserFunc uf = new POUserFunc(new OperatorKey(scope, nig
                .getNextNodeId(scope)), -1, ufInps, new FuncSpec(
                GetMemNumRows.class.getName(), (String[]) null));
        ...
        try {
            ...
            String per = pigContext
                    .getProperties()
                    .getProperty(
                            "pig.skewedjoin.reduce.memusage",
                            String.valueOf(PartitionSkewedKeys.DEFAULT_PERCENT_MEMUSAGE));
            String mc = pigContext.getProperties().getProperty(
                    "pig.skewedjoin.reduce.maxtuple", "0");
            String inputFile = lFile.getFileName();

            return getSamplingJob(sort, prevJob, transformPlans, lFile,
                    sampleFile, rp, null, PartitionSkewedKeys.class.getName(),
                    new String[] { per, mc, inputFile },
     ...
     * The sequence of operations is as following: <li>Transform input sample
     * tuples into another tuple.</li> <li>Add an extra field &quot;all&quot;
     * into the tuple</li> <li>Package all tuples into one bag</li> <li>Add
     * constant field for number of reducers.</li> <li>Sorting the bag</li> <li>
     * Invoke UDF with the number of reducers and the sorted bag.</li> <li>Data
     * generated by UDF is stored into a file.</li>
     * 
     * @param sort
     *            the POSort operator used to sort the bag
     * @param prevJob
     *            previous job of current sampling job
     * @param transformPlans
     ...
     *            path of output file
     * @param rp
     *            configured parallemism
     * @param sortKeyPlans
     *            PhysicalPlans to be set into POSort operator to get sorting
     *            keys
     * @param udfClassName
     *            the class name of UDF
     * @param udfArgs
     *            the arguments of UDF
     * @param sampleLdrClassName
    ...
    private Pair<MapReduceOper, Integer> getSamplingJob(POSort sort,
            MapReduceOper prevJob, List<PhysicalPlan> transformPlans,
            FileSpec lFile, FileSpec sampleFile, int rp,
            List<PhysicalPlan> sortKeyPlans, String udfClassName,
            String[] udfArgs, String sampleLdrClassName) throws PlanException,
            VisitorException {
        ...
        rslargs[0] = (new FuncSpec(Utils.getTmpFileCompressorName(pigContext)))
                .toString();
        // This value is only used by order by. For skewed join, it's calculated
        // based on the file size.
        rslargs[1] = pigContext.getProperties().getProperty(
                PigConfiguration.PIG_RANDOM_SAMPLER_SAMPLE_SIZE, "100");
        ...
        // Now set up a POLocalRearrange which has "all" as the key and the
        // output of the
        ...
        POProject topPrj = new POProject(new OperatorKey(scope, nig
                .getNextNodeId(scope)));
        topPrj.setColumn(1);
        ...
        if (sortKeyPlans != null) {
            ...
            if (sortProjs == null) {
                ...
                for (int i = 0; i < sortProjs.length; i++) {
                    ...
                    if (sortProjs[i].first != null
                            && sortProjs[i].first.isProjectToEnd()) {
                        if (i != sortProjs.length - 1) {
                            // project to end has to be the last sort column
                            throw new AssertionError(
                                    "Project-range to end (x..)"
        ...
        ConstantExpression rpce = new ConstantExpression(new OperatorKey(scope,
                nig.getNextNodeId(scope)));
        ...
        POForEach nfe2 = new POForEach(new OperatorKey(scope, nig
                .getNextNodeId(scope)), -1, genEps, flattened2);
        ...
        POProject prjStar4 = new POProject(new OperatorKey(scope, nig
                .getNextNodeId(scope)));
        ...
        POUserFunc uf = new POUserFunc(new OperatorKey(scope, nig
                .getNextNodeId(scope)), -1, ufInps, new FuncSpec(udfClassName,
                udfArgs));
        ep4.add(uf);
        ...
        POForEach nfe3 = new POForEach(new OperatorKey(scope, nig
                .getNextNodeId(scope)), -1, ep4s, flattened3);
    ...
    static class LastInputStreamingOptimizer extends MROpPlanVisitor {
        ...
        /**
         * indTupIter Look for pattern POPackage->POForEach(if both are
         * flatten), change it to POJoinPackage We can avoid materialize the
         * input and construct the result of join on the fly
         * 
         * @param mr
        ...
        public void visitMROp(MapReduceOper mr) throws VisitorException {
            ...
            // 2. POUnion is the leaf of map plan (so that we exclude distinct,
            // sort...)
            // 3. No combiner plan
            ...
            if (op instanceof POForEach) {
                ...
                for (PhysicalPlan p : planList) {
                    ...
                    try {
                        ...
                        if (proj.getColumn() == pack.getNumInps()) {
                            ...
                            // make sure the project is on a bag which needs to
                            // be
                            // flattened
                            if (!flatten.get(i)
                                    || proj.getResultType() != DataType.BAG) {
                    ...
                    PhysicalOperator succ = p.getSuccessors(proj) != null ? p
                            .getSuccessors(proj).get(0) : null;
                    while (succ != null) {
                        if (!(succ instanceof POProject)) {
                            allSimple = false;
                            break;
                        }
                        // make sure successors of the last project also project
                        // bags
                        ...
                        succ = p.getSuccessors(succ) != null ? p.getSuccessors(
                                succ).get(0) : null;
                    }
                    i++;
                    if (allSimple == false)
                ...
                if (lastInputFlattened && allSimple && projOfLastInput != null) {
                    ...
                    replaceWithPOJoinPackage(mr.reducePlan, mr, pack, forEach,
                            chunkSize);
                }
            }
        }
        ...
        public static void replaceWithPOJoinPackage(PhysicalPlan plan,
                MapReduceOper mr, POPackage pack, POForEach forEach,
                String chunkSize) throws VisitorException {
            String scope = pack.getOperatorKey().scope;
            NodeIdGenerator nig = NodeIdGenerator.getGenerator();
            POJoinPackage joinPackage;
            joinPackage = new POJoinPackage(new OperatorKey(scope, nig
                    .getNextNodeId(scope)), -1, pack, forEach);
    ...
    private static class FindStoreNameVisitor extends PhyPlanVisitor {
        ...
        FindStoreNameVisitor(PhysicalPlan plan, FileSpec newSpec,
                FileSpec oldSpec) {
            super(plan, new DepthFirstWalker<PhysicalOperator, PhysicalPlan>(
                    plan));
